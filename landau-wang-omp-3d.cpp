...
int LW3D(int _L, int _PP_I, int _MAX_MCS_COUNT)  {
    ...
    while(f > f_min)    {
        ...
        double part_of_iteration_time_begin = 0.0, part_of_iteration_time_end = 0.0;
        double time_of_iteration_begin = 0.0, time_of_iteration_end = 0.0;
    ...
    for(double T = 0.01; T <= 8; T += 0.01)  {
        ...
        out_f_td << std::fixed << std::setprecision(8) << T << "\t" << Ut/(L*L*L) << "\t" << Ft/(L*L*L) << "\t" << St/(L*L*L) << "\t" << Ct/(L*L*L) << "\n";
